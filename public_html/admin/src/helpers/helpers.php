		...
		while ($i < strlen($type)) {
			...
			switch ($subtype) {
				...
				case "L" :
					...
					list($tmp_year, $tmp_month, $tmp_day) = jalali_to_gregorian(13112, 12, 1);
...
if (!function_exists('sendSingleSMS')) {
	function sendSingleSMS($phoneNumber, $text) {
		...
		$pass = "123456";
		$domain = "tartansms";
		$param = array('UserName' => $user, 'Pass' => $pass, 'Domain' => $domain, 'SmsText' => $text, 'MobileNumber' => $phoneNumber, 'SenderNumber' => '30006403868611', 'smsMode' => 'SaveInPhone');
		$client->call('SendSingleSms', array('parameters' => $param), '', '', false, true);
		if ($client->fault) {
			return false;
		} else {
			// Check for errors
