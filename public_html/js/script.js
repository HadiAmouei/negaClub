...
var userAgent = navigator.userAgent.toLowerCase(), initialDate = new Date(), $document = $(document),
    ...
    isTouch = "ontouchstart" in window, windowReady = false, isNoviBuilder = false, livedemo = true, plugins = {
    ...
    if (plugins.preloader.length && !isNoviBuilder) {
        pageTransition({
            ...
            conditions: function (event, link) {
                return !/(\#|callto:|tel:|mailto:|:\/\/)/.test(link) && !event.currentTarget.hasAttribute('data-lightgallery') && !event.currentTarget.hasClass('dtp');
            },
    ...
    if (plugins.circleProgress.length) {
        for (var i = 0; i < plugins.circleProgress.length; i++) {
            ...
            circle.circleProgress({
                ...
                fill: {gradient: circle.attr('data-gradient').split(","), gradientAngle: Math.PI / 4},
...
$(function () {
    ...
    if (plugins.dateCountdown.length) {
        for (var i = 0; i < plugins.dateCountdown.length; i++) {
            var
                dateCountdownItem = $(plugins.dateCountdown[i]), countdownRender = function () {
                    dateCountdownItem.TimeCircles({time: {Seconds: {show: !(window.innerWidth < 768),}}}).rebuild();
                };
